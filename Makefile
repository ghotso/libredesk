...
run-backend:
	...
	CGO_ENABLED=0 go run -ldflags="-s -w -X 'main.buildString=${BUILDSTR}' -X 'main.versionString=${VERSION}' -X 'github.com/ghotso/libredesk/internal/version.Version=${VERSION}' -X 'main.frontendDir=frontend/dist'" cmd/*.go
...
build-backend: $(STUFFBIN)
	...
	@CGO_ENABLED=0 go build -a \
		-ldflags="-X 'main.buildString=${BUILDSTR}' -X 'main.versionString=${VERSION}' -X 'github.com/ghotso/libredesk/internal/version.Version=${VERSION}' -s -w" \
		-o ${BIN} cmd/*.go
